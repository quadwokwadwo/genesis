...
const AddInventoryItem: React.FC = () => {
    ...
    const deleteItem = (itemData: TInventoryItem) => {
        confirmDialog({
            ...
            accept: async () => {
                try {
                    setStateValue({ loading: true });
                    const response = await inventory.removeItem(itemData.itemId!);
    ...
    const saveItem = async () => {
        ...
        try {
            ...
            if (response.status === 200 && response.operatedData) {
                ...
                setStateValue({
                    showItemDialog: false,
                    item: { ...state.item, itemId: 0, itemName: '', description: '', categoryId: null, brandId: null, unitPrice: null, quantityInStock: 0, reorderLevel: 10, unitsPerBlister: 0, packagingType: PackingType.other }
                });
                await fetchInventoryItems();
            }
        } catch (error) {
    ...
    const onItemsUpload = async (e: FileUploadFilesEvent) => {
        try {
            ...
            const uploadFileHeaders = ['Item Name', 'Category', 'Brand', 'Unit Price', 'Quantity', 'Reorder Level', 'Units per Blister', 'Packaging Type', 'Description'];
            const uploadObjectNames = ['itemName', 'category', 'brand', 'unitPrice', 'quantityInStock', 'reorderLevel', 'unitsPerBlister', 'packagingType', 'description'];
            console.log(file);
            const uploadData = await configureExcelUpload<TInventoryItem>(file, uploadFileHeaders, uploadObjectNames);
            console.log(uploadData);

            const response = await inventory.uploadInventoryItems(uploadData);
            console.log(response);
            if (response.operatedData === 1) {
                toast.current?.show({
                    severity: 'success',
                    summary: 'File Upload Success',
                    detail: `Inventory drugs were successfully uploaded from the file`
                });
                await fetchInventoryItems();
            } else {
                toast.current?.show({
